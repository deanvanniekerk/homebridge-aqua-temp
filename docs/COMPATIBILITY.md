...
Other device models, Node majors, Homebridge 1.x and Windows are not claimed as supported. Homebridge UI is host tooling rather than a plugin dependency. Check other installed plugins before upgrading a shared Homebridge runtime.
...
| Mode | Wire mode | Target field | Profile range | Home range |
| ---- | --------- | ------------ | ------------- | ---------- |
...
Unknown profiles remain disabled; unsupported/missing capabilities do not stop account polling or sibling sensors. Missing core readings can still disable dependent thermostat controls. Generic faults are not decoded into specific protection diagnoses.

## Release confidence

The initial release is a beta. Owner feedback reports the installed controls looking good, and issue #10 is closed. No seven-day physical soak, complete range sweep or all-model firmware validation is recorded. Stable reliability claims require a separate evidence review; see [validation](VALIDATION.md) and [release process](RELEASING.md).
