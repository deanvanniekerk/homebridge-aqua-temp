...
Sanitized evidence was retained privately as `cool-target-mode-result.json`, `cool-target-before-write.json`, `cool-target-up-result.json` and `cool-target-up-readback.json`. No request remains armed. Owner app confirmation, restoration of Cool target 8, restoration to Heat and helper cleanup remain pending. The helper remains temporarily loaded for those authorized steps. No Power or Set_Temp command was sent; this single observed target value does not establish the full Cool range or active cooling behavior.
