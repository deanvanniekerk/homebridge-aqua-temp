...
The upward result/readback were preserved privately as `auto-target-up-result.json` and `auto-target-up-readback.json`. No request remains armed. Owner app confirmation, restoration of Auto target 30, restoration to Heat and helper cleanup are pending. The helper remains temporarily loaded for those authorized steps. No Power command or Set_Temp write was sent.
