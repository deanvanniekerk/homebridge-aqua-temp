...
Aqua Temp for Homebridge exposes supported cloud-connected heat pumps to Apple Home. It is an independent implementation; source, tests and assets from other integrations must not be copied.

## User behavior

- One thermostat per supported selected device, with Off/Heat/Cool/Auto and a single mode-specific target.
...
A publication requires a separate maintainer release decision. Prepare a beta while sustained hardware evidence is incomplete. Stable reliability claims require actual-host validation and a documented seven-day run, not just simulated recovery or a closed issue. See [release process](RELEASING.md) and [validation](VALIDATION.md).
