...
- Heat target commands write only absolute `R02` values from 15–40°C on a 0.5°C grid. The app limits and adjacent half-degree steps were observed. Earlier API experiments accepted integer R02 writes and showed persistent 31→32°C readback. On 2026-09-09, the installed iHost plugin completed an owner-coordinated 32 → 32.5 → 32°C target-only round trip with power Off. Both writes succeeded with R02-backed HAP readback and independent owner confirmation in Aqua Temp. The setter still requires matching R02 readback before success. No claim is made that every grid point was physically tested.
...
Physical validation is tracked in #10. Installation, pairing and one fractional target round trip have been exercised; app-originated target propagation to HAP has also been observed; isolated outage recovery and sustained operation remain pending. Unknown activity deliberately produces a HAP communication error, and owner screenshots confirm that Apple Home marks the whole tile unavailable in that situation. Npm distribution remains in #11. These limitations are not hidden prerequisites for closing the implemented local model, adapter and automated-validation issues.
