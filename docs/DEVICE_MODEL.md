...
Actual iHost installation, Apple Home presentation (including unknown running activity), physical fractional API writes and sustained hardware operation remain in #10. Unknown activity deliberately produces a HAP communication error; whether Apple Home marks the whole tile unavailable in that situation is a hardware presentation limitation still to test. Npm distribution remains in #11. These limitations are not hidden prerequisites for closing the implemented local model, adapter and automated-validation issues.
