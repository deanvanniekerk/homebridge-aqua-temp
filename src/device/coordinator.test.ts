...
import { discoverDevices, normalizeReadings } from './device-model.js';
...
test('slow update consumers receive the latest state and cancellation releases their listeners', async () => {
  const scheduler = new FakeScheduler();
  let reads = 0;
  const coordinator = new AccountCoordinator(
    {
  ...
  onTestFinished(() => coordinator.close());
