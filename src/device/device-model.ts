...
const discoveryListSchema = z.array(z.unknown()).max(10_000);
const discoveryRecordSchema = z.looseObject({
  deviceCode: z.unknown().optional(),
  device_code: z.unknown().optional(),
});
const parameterListSchema = z.array(z.unknown()).max(1024);
const parameterRowSchema = z.looseObject({
  code: z.string().refine(identifier),
});
...
export function discoverDevices(
  ...
  const parsedOwned = discoveryListSchema.safeParse(owned);
  const parsedShared = discoveryListSchema.safeParse(shared);
  if (
    !parsedOwned.success ||
    !parsedShared.success ||
    parsedOwned.data.length + parsedShared.data.length > 10_000
  ) {
  ...
  const lists: [unknown[], 'owned' | 'shared'][] = [
    [parsedOwned.data, 'owned'],
    [parsedShared.data, 'shared'],
  ];
  for (const [list, source] of lists) {
    for (const value of list) {
      const parsedRecord = discoveryRecordSchema.safeParse(value);
      if (!parsedRecord.success) {
        rejectedRecords += 1;
        continue;
      }
      const record = parsedRecord.data;
      const id: unknown = record.deviceCode ?? record.device_code;
      if (
        !identifier(id) ||
        ('deviceCode' in record &&
          'device_code' in record &&
          record.deviceCode !== record.device_code)
      ) {
        rejectedRecords += 1;
        continue;
      }
      const profile =
        record.model === 'PASRW040-P-BP4II-C' && record.custModel === 'BOOSTi-INV-HP-40'
          ? 'boost-i-hp40'
          : 'unknown';
...
function parameters(input: unknown): Map<string, unknown> {
  const parsed = parameterListSchema.safeParse(input);
  if (!parsed.success) throw new DeviceError('invalid-response');
  ...
  for (const value of parsed.data) {
    ...
    const row = parameterRowSchema.safeParse(value);
    if (!row.success) continue;
    // Duplicate selectors cannot establish which value is current, even if equal.
    result.set(row.data.code, result.has(row.data.code) ? null : row.data);
  }
  return result;
}
