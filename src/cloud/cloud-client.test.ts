...
import { credentials, login, reply, serverFor, success } from './fake-cloud.test-support.js';
...
test('a hanging login is bounded and cancelling one waiter does not cancel its peers', {
  timeout: 10_000,
}, async (t) => {
...
test('malformed envelopes and missing or unsafe tokens fail closed without exposing vendor data', async (t) => {
  ...
  for (const body of cases) {
    ...
    onTestFinished(() => client.close());
    await assert.rejects(
      client.read({ kind: 'owned' }),
      (error: { category?: string; cause?: unknown }) => {
...
test('read and write deadlines include shared authentication and prevent late command delivery', {
  timeout: 10_000,
}, async (t) => {
  let release: (() => void) | undefined;
  const arrived = new Promise((resolve) => {
...
test('shutdown cancels outstanding sockets and backoff without sending further requests', {
  timeout: 10_000,
}, async (t) => {
...
test('connection refusal and TLS negotiation failures return sanitized, bounded transport errors', async () => {
  ...
  for (const origin of [
    `http://127.0.0.1:${port}`,
    `https://127.0.0.1:${(tls.address() as AddressInfo).port}`,
  ]) {
...
test('DNS lookup failure traverses the real HTTPS transport without disclosing resolver errors', async () => {
  const original = dns.lookup;
  let lookups = 0;
  dns.lookup = ((hostname, _options, callback) => {
