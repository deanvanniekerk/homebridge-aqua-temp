...
import { CommandError } from '../device/command-queue.js';
import type { AccountCoordinator } from '../device/coordinator.js';
import type { DeviceCommand, OperatingMode, Reading } from '../device/device-model.js';
...
export class Thermostat {
  ...
  private readings() {
    ...
    if (state?.status !== 'healthy' || state.readings?.connectivity !== 'online')
      throw this.unavailable();
  ...
  private range(): Range | undefined {
    let control: { minimumCelsius: number; maximumCelsius: number; stepCelsius: number };
    try {
