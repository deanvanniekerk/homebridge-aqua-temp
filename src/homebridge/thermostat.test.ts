...
import { deferred, FakeScheduler } from '../device/fake-scheduler.test-support.js';
...
async function setup(
  ...
  options: {
    ...
    write?: (command: unknown) => Promise<void> | void;
    startBeforeAccessory?: boolean;
    legacyReadOnlyCache?: boolean;
  } = {},
) {
  ...
  if (options.legacyReadOnlyCache)
    accessory
      ...
      .setProps({ perms: [api.hap.Perms.EVENTS, api.hap.Perms.PAIRED_READ] });
...
test('newly recognized modes never masquerade as Heat and missing target does not block Off', async (t) => {
  ...
  for (const mode of ['cool', 'auto']) {
    ...
    assert.ok(
      h.get('TargetHeatingCoolingState').props.perms.includes(h.api.hap.Perms.PAIRED_WRITE),
    );
    await h.get('TargetHeatingCoolingState').handleSetRequest(0);
    assert.equal(await h.get('TargetHeatingCoolingState').handleGetRequest(), 0);
  }
...
test('legacy cached read-only target-state permissions are restored on upgrade', async (t) => {
  ...
  await h.start();
  assert.ok(h.get('TargetHeatingCoolingState').props.perms.includes(h.api.hap.Perms.PAIRED_WRITE));
  await h.get('TargetHeatingCoolingState').handleSetRequest(0);
  assert.equal(await h.get('TargetHeatingCoolingState').handleGetRequest(), 0);
});
