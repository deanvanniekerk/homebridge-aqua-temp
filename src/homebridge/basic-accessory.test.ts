...
import { AccountCoordinator } from '../device/coordinator.js';
import { FakeScheduler } from '../device/fake-scheduler.test-support.js';
import { BasicAccessory } from './basic-accessory.js';

const yes = (value) => ({ available: true, value });
const unknown = { available: false, reason: 'unverified' };
type TemperatureReading = { available: boolean; value?: number; reason?: string };
test('independent inlet, outlet and ambient sensors isolate missing readings and recover after stale data', async () => {
  ...
  const sample = {
    ...
    outletCelsius: yes(23) as TemperatureReading,
  ...
  const sensors = ['inlet', 'outlet', 'ambient'].map((role) => {
    ...
    assert.equal(characteristic.props.perms.includes(api.hap.Perms.PAIRED_WRITE), false);
