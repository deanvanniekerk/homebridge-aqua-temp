...
async function host(t, extraConfig = {}, cached = [], scenario = 'online') {
  ...
  const hostEvents: EventEmitter = api;
  hostEvents.on('registerPlatformAccessories', (accessories) => {
  ...
  hostEvents.on('unregisterPlatformAccessories', (accessories) => {
  ...
  hostEvents.on('updatePlatformAccessories', (accessories) => updated.push(...accessories));
...
test('an unconfigured platform removes stale accessories without starting monitoring', async (t) => {
  ...
  const logs = [];
  const hostEvents: EventEmitter = api;
  hostEvents.on('registerPlatformAccessories', () => assert.fail('registered an accessory'));
  hostEvents.on('unregisterPlatformAccessories', (accessories) => removed.push(...accessories));
  const log = Object.fromEntries(
